...
jupyter: julia-1.9
=======
>>>>>>> ddae644ba564dd303f505556c558107fcb0e4a57
date: 2024-01-26
week: 3
categories: [Lab]
...
```{julia}
function get_depth_damage_function(depth_train::Vector{<:T}, dmg_train::Vector{<:AbstractFloat}) where {T<:Unitful.Length}
...
p = let
    ...
    scatter(depths, damages, xlabel="Flood Depth", ylabel="Damage (%)", label="$(dd.description) ($(dd.source))", legend=:bottomright, size=(800, 400), linewidth=2)
end
p
```
...
```{julia}
p1 = plot(gauge_dist, label="Gauge Distribution", xlabel="Water Level (ft)", ylabel="Probability Density", legend=:topright, linewidth=2)
```
